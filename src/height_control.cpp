...
void heightReading(const ardrone_autonomy::Navdata& msg_in)
{
    ...
    if(otomatis)
    {
        if(drone_altd<950)m='i';
        else if(drone_altd>1050)m='k';
        else
        {
            m='h';
...
int main(int argc, char** argv)
{
    ...
    ros::ServiceClient toggleCam = node.serviceClient<std_srvs::Empty>("/ardrone/togglecam");
